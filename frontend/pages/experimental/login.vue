...
const login = async () => {
  const result = await postLogin(credentials.value.username, credentials.value.password).then(navigateTo('experimental/meal-page/'))
  if (result.status.value == 'error') {
    error.value = true
  }else{
    // navigateTo('experimental/meal-page/')
    console.log(result)
  }
}
</script>
  ...
  <div class="card">
    ...
    <form @submit.prevent>
      <div class="field">
        ...
        <input autocomplete="off" id="logemail" v-model="credentials.username" placeholder="Email" class="input-field" name="logemail" type="text">
      </div>
      <div class="field">
        <svg class="input-icon" viewBox="0 0 500 500" xmlns="http://www.w3.org/2000/svg">
