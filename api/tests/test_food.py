...
class FoodTestCase(BasicTestCase):
    ...
    def test_Food_get_list(self):
        ...
        self.assertEqual(response.data['count'], self.number_of_foods)
    ...
    def test_delete_Food(self):
        ...
        response = self.client.get(f'{self.base_url}/')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['count'], self.number_of_foods - 1)
    ...
    def test_update_pet_parameter(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.check_equality(response.data, self.new_Food)     # check if the new_pet data is the same as response.data
